...
%features = [features(:,1:3),features(:,5:end)];
...
for iNumFeaturesUsed = 1:nTotalFeatures  
  ...
  for iFeature = 1:nAvailableFeatures
    ...
    for i = 1:10
        ...
        tmpFeatures = features(train,tmpSelected);
        minFeatures = min(tmpFeatures);
        maxFeatures = max(tmpFeatures);
        
        testFeatures = features(test,tmpSelected);
        isWrongClass = ClassifyMinMax(testFeatures, minFeatures, ...
          maxFeatures) ~= key(test);
        tmpError = tmpError + sum(isWrongClass)/length(key(test));
    end
...
title('Classification rates from forward selection, MinMax',...
